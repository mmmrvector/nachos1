...
Thread::Thread(char* threadName, int pri = 1000)
{
    name = threadName;
    stackTop = NULL;
    stack = NULL;
	...
	if(tid == -1) //the number of threads has reached limit
	{
		printf("no more thread can be created\n");
		//return;
	}
	//Vector
	//allocate user id
...
Thread::Sleep ()
{
    ...
    status = BLOCKED;
	//Vector
	//put the blocked thread into queue
	(scheduler->blockedList)->Append((void*)this);
