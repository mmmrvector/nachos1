...
//		interrupts are re-enabled
//		a user instruction is executed  /idle
//----------------------------------------------------------------------
void
Interrupt::OneTick()
{
...
// check any pending interrupts are now ready to fire
	...
	//inHandler = FALSE;			// for a context switch, ok to do it now
	yieldOnReturn = FALSE;
...
Interrupt::Halt()
{
	...
	printf("page hit:%d\n", machine->pagehit);
	printf("page miss:%d\n", machine->pagemiss);
	printf("rate:%f\n",(double)( machine->pagehit)/(machine->pagehit + machine->pagemiss));
    stats->Print();
...
Interrupt::CheckIfDue(bool advanceClock)
{
    MachineStatus old = status;
    int when;
 //printf("in checkIfDue\(\), Intstatus is %d\n", getLevel());
...
//vector
void
Interrupt::TurnOffInterrupt()
{
	ChangeLevel(IntOn,IntOff);
}
void 
Interrupt::TurnOnInterrupt()
{
